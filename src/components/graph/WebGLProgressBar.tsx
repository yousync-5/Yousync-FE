...
interface WebGLProgressBarProps {
...
export default function WebGLProgressBar({ 
  value, 
  width = 400, 
  height = 20, 
  theme = 'dark',
  ...
  const initWebGL = useCallback(() => {
    ...
    const fragmentShaderSource = `
      ...
      void main() {
        // 글로우 효과
        float glow = 0.4 * sin(gl_FragCoord.x * 0.05 + v_time * 2.0) + 0.6;
        
        // 파티클 효과
        float particle = sin(gl_FragCoord.x * 0.1 + v_time * 3.0) * 0.1 + 0.9;
        
        vec3 finalColor = v_color * glow * particle;
  ...
  const renderProgressBar = useCallback(() => {
    ...
    // 진행도 바 배경 그리기
    drawProgressBackground(gl, program, colors);
  ...
  // 진행도 바 배경 그리기
  const drawProgressBackground = (gl: WebGLRenderingContext, program: WebGLProgram, colors: any) => {
    const bgColor = [
    ...
    gl.vertexAttribPointer(colorLocation, 3, gl.FLOAT, false, 28, 8);
    gl.vertexAttribPointer(alphaLocation, 1, gl.FLOAT, false, 28, 20);
    gl.vertexAttribPointer(timeLocation, 1, gl.FLOAT, false, 28, 24);
  ...
  const drawProgressFill = (gl: WebGLRenderingContext, program: WebGLProgram, colors: any, fillValue: number, currentTime: number) => {
    ...
    // 채우기 너비 계산
    const fillWidth = fillValue * 1.9; // -0.95에서 0.95까지

    const vertices = new Float32Array([
      -0.95, -0.3, ...fillColor, 0.9, currentTime,
      -0.95, 0.3, ...fillColor, 0.9, currentTime,
      -0.95 + fillWidth, 0.3, ...fillColor, 0.9, currentTime,
      -0.95 + fillWidth, -0.3, ...fillColor, 0.9, currentTime
    ]);
  ...
  const drawGlowEffect = (gl: WebGLRenderingContext, program: WebGLProgram, fillValue: number, currentTime: number) => {
    ...
    const vertices = new Float32Array([
      -0.95, -0.45, ...glowColor, glowIntensity, currentTime,
      -0.95, 0.45, ...glowColor, glowIntensity, currentTime,
      -0.95 + fillWidth, 0.45, ...glowColor, glowIntensity, currentTime,
      -0.95 + fillWidth, -0.45, ...glowColor, glowIntensity, currentTime
    ]);
  ...
  const animate = useCallback(() => {
    ...
    renderProgressBar();
    animationRef.current = requestAnimationFrame(animate);
  }, [renderProgressBar, animation, isClient]);
  ...
  }, [initWebGL, renderProgressBar, animate, animation, isClient]);
  ...
  }, [animatedValue, renderProgressBar, isClient]);
