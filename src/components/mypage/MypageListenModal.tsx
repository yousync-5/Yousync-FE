...
import { TokenDetailResponse } from '@/types/pitch';
...
const MypageListenModal: React.FC<DubbingListenModalProps> = ({ open, onClose, modalId, tokenId }) => {
  const videoId: string | undefined = modalId;
  ...
  useEffect(() => {
    const fetchTokenDetail = async () => {
      try {
        ...
        console.error('토큰 디테일 불러오기 실패', err);
      }
    };
  ...
  const getSynthesizeAudio = async () => {
    try {
      ...
      const res = await axios.get<SynthesizeAudioResponse>(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/tokens/${tokenId}/latest-dubbing/`,
    ...
    if (typeof finalStartTime === 'number' && !isNaN(finalStartTime)) {
      ...
      setTimeout(() => {
        if (youtubePlayerRef.current) {
          ...
          // 오디오도 함께 재생
          if (audioRef.current && audioResponse?.url) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        ...
        {audioResponse?.url ? (
          <audio ref={audioRef} src={audioResponse.url} />
        ) : (
          <audio ref={audioRef} />
        )}
...
export default MypageListenModal;